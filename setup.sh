...
set_fstab() {
    ...
    entry="$rootUUID / $filesystem rw,ssd,noatime,autodefrag,compression=lz4 0 0"
...
setup_bootloader() {
    ...
    if grep -q gummiboot $f; then
        install_gummiboot
    elif grep -q syslinux $f; then
        install_syslinux
    elif grep -q limine $f; then
        install_limine
    elif grep -q grub $f; then
        install_grub
    elif grep -q rEFInd $f; then
        install_refind
    elif grep -q clover $f; then
        install_clover
    fi
